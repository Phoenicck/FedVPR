...
def get_dataloaders(client_num, data_root, seed, param = {'dataset': 'Bloodmnist', 'Known_class': 5, 'unKnown_class': 3, 'Rotation': 45, 'Resize': 144, 'CropSize':128, 'Batchsize': 8, 'dirichlet':1.0}):
    ...
    if dataset == 'Bloodmnist':
        total_class = 8
    if dataset == 'Organcmnist' or dataset == 'OrganMNIST3D':
        total_class = 11
    ...
    img_list = np.concatenate([trainx, valx, testx], axis = 0)
    label_list = np.concatenate([trainy, valy, testy], axis = 0)
    ...
    for i in range(client_num): 
        ...
        client_trainloader=torch.utils.data.DataLoader(client_trainset, batch_size=batchsize, shuffle=True, drop_last=True, **loader_kwargs)
...
class MedMINIST(Dataset):
    def __init__(self, data_root, data_index, setname, datax, datay, param = {'Known_class': 6, 'unKnown_class': 9, 'Rotation': 45, 'Resize': 144, 'CropSize':128}):
        ...
        self.transform_train = Transform3D()
        self.transform_test = Transform3D()        
...
if __name__ == '__main__':
    datadir = './dataset/organmnist3d.npz'
    def set_seed(seed):
        import random
