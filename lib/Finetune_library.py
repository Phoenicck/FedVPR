...
def train(args, device, epoch, net, trainloader, optimizer, net_peers=None, attack = None, unknown_dis = None):
    ...
    for batch_idx, (inputs, targets, img_dirs) in enumerate(trainloader):
        ...
        if epoch>=0:      
            ...
            if (is_boundary.sum()>0 and (args.dataset=='Hyperkvasir' or args.dataset=='RetinalOCT')) or (is_boundary.sum()>1 and (args.dataset=='Bloodmnist' or args.dataset=='OrganMNIST3D')): #  batchnorm error when batchsize = 1               
                ...
                if inputs_unknown is not None:                                        
                    ...
                    if epoch in args.start_epoch:
                        ...
                        for index in range(len(targets_unknown)):
                            if ((args.dataset=='Hyperkvasir' or args.dataset=='RetinalOCT') and PDs[index]>0) or ((args.dataset=='Bloodmnist' or args.dataset=='OrganMNIST3D') and PDs[index]>-1):
