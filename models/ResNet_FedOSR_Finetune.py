...
if __name__=='__main__':
    ...
    net.maxpool = torch.nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
    from acsconv.converters import ACSConverter
    net = ACSConverter(net)    
    y = net(x)
    print(y['discrete_feats'].shape)
    p = net.discrete_forward(y['discrete_feats'])
    print(p)